...
def main():
    ...
    n_train = len(paths_left) + len(paths_right) + len(paths_stop) + len(paths_other) + len(paths_back) + len(paths_red)+ len(paths_green)
    ...
    # 讀取圖片到資料集
    paths_train = paths_left + paths_right + paths_stop + paths_other + paths_back + paths_red + paths_green
    ...
    begin_ind = n_left + n_right + n_stop + n_back
    end_ind = n_left + n_right + n_stop + n_back + n_other
    label[begin_ind:end_ind, 4] = 1.0

    begin_ind = n_left + n_right + n_stop + n_back + n_red
    end_ind = n_left + n_right + n_stop + n_back + n_red + n_other
    label[begin_ind:end_ind, 5] = 1.0

    begin_ind = n_left + n_right + n_stop + n_back + n_red + n_green
    end_ind = n_left + n_right + n_stop + n_back + n_red + n_green + n_other
